...
namespace GGEZ
{
    ...
    public class PixelPerfect_camera : MonoBehaviour
    {
        ...
        public float base_OffsetX;
        public float base_OffsetY;
        [Space]
        public float heigh_Offset;
        [Space]
        public float upper_limit;
        public float upper_offset;
        [Space]
        ...
        void Update()
        {
            targetPos = new Vector3(followTarget.transform.position.x + base_OffsetX, base_OffsetY + heigh_Offset, transform.position.z);
            ...
            if (followTarget.transform.position.y > upper_limit)
            {
                heigh_Offset = followTarget.transform.position.y + upper_offset;
            }
            else if (followTarget.transform.position.y < lower_limit)
            {
                heigh_Offset = followTarget.transform.position.y + lower_offset;
            }
            else
            {
                heigh_Offset = 0f;
        ...
        void LateUpdate()
        {
            ...
            float halfPixelOffsetInWorldUnits =snapSizeWorldUnits;
            ...
            first = -halfWidth + pixelPerfectXOffset;
            second = halfWidth + pixelPerfectXOffset;
            third = -halfHeight + pixelPerfectYOffset;
            fourth = halfHeight + pixelPerfectYOffset;
